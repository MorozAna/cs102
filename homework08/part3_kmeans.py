...
if __name__ == "__main__":
    ...
    X = df.loc[:, df.columns != "Species"].as_matrix()
    model = KMeans(3)
    model.fit(X)
    ...
    # Plot Sepal
    plt.subplot(1, 2, 1)
    plt.scatter(df.SepalLengthCm, df.SepalWidthCm, c=colormap[clusters], s=40)
    plt.title("Sepal")  # Чашелистик
    plt.show()
    plt.subplot(1, 2, 2)
    plt.scatter(df.PetalLengthCm, df.PetalWidthCm, c=colormap[clusters], s=40)
    plt.title("Petal")  # Лепесток
    plt.show()
